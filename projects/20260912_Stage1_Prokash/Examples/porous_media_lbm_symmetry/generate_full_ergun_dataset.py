"""
Generate a synthetic porous-media dataset spanning the ENTIRE Ergun curve
— deep-viscous (Darcy) branch, crossover, and inertial (Forchheimer)
plateau — in one CSV, mirroring generate_viscous_dataset.py /
generate_inertial_dataset.py.

This is the input for the region-free local-symmetry scan
(discover_local_symmetry.py): unlike the two wide-phi single-branch
datasets, NOTHING here tells the pipeline where one regime ends and the
other begins.  The full textbook Ergun formula

    f = [ 150*(1-phi)/Re_p + 1.75 ] * (1-phi) / phi^3

is evaluated over Re_p in [1e-6, 1e6] (12 decades, crossing the
viscous/inertial transition at Re_p ~ 85*(1-phi)) and a WIDE porosity
sweep, with multiplicative log-normal noise on f and log-normal jitter
on Re_p — the same noise model as the per-branch generators.
...
Usage:  python generate_full_ergun_dataset.py
Output: dataset_ergun_full_curve.csv
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate full-curve synthetic Ergun dataset "
                    "(viscous + crossover + inertial)")
    ...
    parser.add_argument("--output", default="dataset_ergun_full_curve.csv")
    ...
    print(f"Full Ergun curve: Re_p in [1e{args.re_min_exp:.0f}, "
    ...
    Re_c = A_VISC * (1 - np.array(phis)) / B_INER
    print(f"Viscous/inertial crossover Re_p (viscous term = inertial term): "
          f"{Re_c.min():.1f} .. {Re_c.max():.1f}")
    ...
    for phv in phis:
        for dv in ds:
            for Re0 in Re_grid:
                ...
                rows.append({
                    "filename": f"ergun_full_phi{phv:.3f}_d{int(dv)}_Re{Re0:.1e}",
